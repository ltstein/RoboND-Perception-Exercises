...
def compute_color_histograms(cloud, using_hsv=True):
    ...
    hist_channel_features = np.concatenate((h_hist[0], s_hist[0], v_hist[0])).astype(np.float64)
    # Generate random features for demo mode.  
    # Replace normed_features with your feature vector
    normed_features = hist_channel_features / np.sum(hist_channel_features)
...
def compute_normal_histograms(normal_cloud):
    ...
    # TODO: Concatenate and normalize the histograms
    hist_norm_features = np.concatenate((x_hist[0], y_hist[0], z_hist[0])).astype(np.float64)
    # Generate random features for demo mode.  
    # Replace normed_features with your feature vector
    normed_features = hist_norm_features / np.sum(hist_norm_features)
